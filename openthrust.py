...
class OpenThrust:
    ...
    def __init__(self, stkcode, ds='csv'):
        ...
        self.allowcloseinday = False  # 允许日内平仓
        self.maxopentimes = 100000  # 每日最大开仓次数
    """
    策略回测执行
    """
    def exec(self):
        ...
        hqlist_day = MA(hqlist_day, 10)
        hqlist_day = MA(hqlist_day, 20)
        hqlist_day = MAZF(hqlist_day, 30)
        hqlist_day = MAZF(hqlist_day, 60)
        ...
        for i in range(len(self.tickseries)):  # 循环读取
            ...
            if i == 0:
                opentimes = 0  # 今日最大开仓次数
            elif date != self.tickseries[i-1]['date']:
                opentimes = 0
            ...
            for j in range(len(hqlist_day)):
                if hqlist_day[j]['date'] == date:
                    ...
                    if j == 0:
                        hbond = 9999
                        lbond = 1
                        # ma_short = float(hqlist_day[j]['MA10'])
                        # ma_long = float(hqlist_day[j]['MA20'])
                        mazf_short = float(hqlist_day[j]['MAZF30'])  # 平均日内振幅
                        mazf_long = float(hqlist_day[j]['MAZF60'])  # 平均日内振幅
                    else:
                        atr = float(hqlist_day[j-1]['ATR'])
                        # ma_short = float(hqlist_day[j-1]['MA10'])
                        # ma_long = float(hqlist_day[j-1]['MA20'])
                        mazf_short = float(hqlist_day[j-1]['MAZF30'])  # 平均日内振幅
                        mazf_long = float(hqlist_day[j-1]['MAZF60'])  # 平均日内振幅
                        # hbond = todayopen * (1 + 1 / 100000)
                        # lbond = todayopen * (1 - 1 / 100000)
                        hbond = todayopen + self.ATRmults * atr
                        lbond = todayopen - self.ATRmults * atr
                    break
            ...
            # 开多条件
            if self.obj_PM.get_currdirect() == 0 and (H1 < hbond) and (H >= hbond) and (L < hbond) and (opentimes <= self.maxopentimes):  # 开多
                self.obj_PM.long(hbond)  # 开多
                self.obj_PM.set_stopprice(lbond)
                opentimes = opentimes + 1
            # 开空条件
            elif self.obj_PM.get_currdirect() == 0 and (L1 > lbond) and (L <= lbond) and (H > lbond) and self.allowshort and (opentimes <= self.maxopentimes):  # and (ma_long > ma_short):  # 开空
