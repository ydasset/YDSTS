...
class Donchian:
    ...
    def exec(self):
        ...
        hqlist = DONCHIAN(self.tickseries,  self.lens)
        ...
        for i in range(len(self.tickseries)):  # 循环读取
            ...
            # 读取高级别指标
            hbond =hqlist[i]['dochianhigh']
            lbond = hqlist[i]['donchinalow']
            ...
            if self.isdaytrade and (time < "09:35:00" or time >= "14:55:00"):  # 开盘和收盘附近15分钟不开仓
                continue
            # 开多条件
            if self.obj_PM.get_currdirect() == 0 and (H >= hbond) and (L < hbond):  # 开多
                self.obj_PM.long(hbond)  # 开多
                self.obj_PM.set_stopprice(lbond)
            # 开空条件
            elif self.obj_PM.get_currdirect() == 0 and (L <= lbond) and (H > lbond) and self.allowshort:  # 开空
