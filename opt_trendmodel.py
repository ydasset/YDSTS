...
class OptTrendModel:
    ...
    def __init__(self, stkcode, feemod=None, begindate='', enddate='', ds='csv'):
        ...
        """
        self.bars = 1  # 指标参数
        self.ma1len_short = 15  # MA短线长度
        self.ma1len_long = 30  # MA长线长度
        self.ma2len_short = 60  # MA短线长度（长周期）
        self.ma2len_long = 120  # MA长线长度（长周期）
        self.wrlen = 5  # WR长度
    ...
    def exec(self):
        ...
        for i in range(len(self.tickseries)):  # 循环读取
            ...
            # 2、MA双线呈空头排列（短下长上）
            # 3、WR进入超买区
            elif self.allowshort and ma1short1 < ma1long1:# and wrval2 < self.overbought <= wrval1:
                if self.obj_PM.get_currdirect() == 0 and self.obj_PM_model.get_currdirect() == 0:  # 实际开仓
