    ...
    def exec(self):
        ...
        for i in range(len(self.tickseries)):  # 循环读取
            ...
            if self.isdaytrade and (time < "09:35:00" or time >= "14:54:00"):  # 开盘和收盘附近15分钟不开仓
                continue
            # 开多条件
            if self.obj_PM.get_currdirect() != -1 and (L1 > prehbond) and (H > hbond) and (L <= hbond) and self.allowshort:  # 开空
                if self.obj_PM.get_currdirect() == 1:
                    self.obj_PM.closeposition(hbond)
                self.obj_PM.short(hbond)  # 开空
                self.obj_PM.set_stopprice(hbond * (1 + self.stoprate / 100))
            # 开空条件
            elif self.obj_PM.get_currdirect() != 1 and (H1 < prelbond) and (L < lbond) and (H >= lbond):  # 开多
                if self.obj_PM.get_currdirect() == -1:
